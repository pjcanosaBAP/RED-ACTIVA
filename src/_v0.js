...
function exportBackupJSON(){
  if(!saved.length && !autoevals.length){showToast('⚠️ No hay auditorías para exportar');return;}
  const payload = { saved, autoevals };
  ...
  showToast('💾 Backup exportado: ' + saved.length + ' EV + ' + autoevals.length + ' AE');
}
...
function importBackupJSON(input){
  ...
  reader.onload = function(e){
    try {
      ...
      // Support both old format (plain array) and new format ({saved:[], autoevals:[]})
      const evList  = Array.isArray(raw) ? raw : (Array.isArray(raw.saved)     ? raw.saved     : []);
      const aeList  = Array.isArray(raw) ? []  : (Array.isArray(raw.autoevals) ? raw.autoevals : []);

      let added=0, replaced=0, skipped=0, aeAdded=0, aeReplaced=0;

      evList.forEach(function(a){
        if(!a.distribuidor || !a.answers) { skipped++; return; }
        const idx = saved.findIndex(s=>s.distribuidor===a.distribuidor && s.edicion===a.edicion);
        if(idx>=0){ saved[idx]=a; replaced++; } else { saved.push(a); added++; }
      });

      aeList.forEach(function(a){
        if(!a.distribuidor || !a.answers) return;
        const idx = autoevals.findIndex(s=>s.distribuidor===a.distribuidor && s.edicion===a.edicion);
        if(idx>=0){ autoevals[idx]=a; aeReplaced++; } else { autoevals.push(a); aeAdded++; }
      });
      ...
      renderGlobal();
      input.value = '';
      const msg = '✅ Importado: '+added+' EV nuevas, '+replaced+' reemplazadas'
        +(aeAdded||aeReplaced ? ' | '+aeAdded+' AE nuevas, '+aeReplaced+' reemplazadas' : '')
...
function mergeSnapshotFromHTML(html, label){
  ...
  try{
    ...
    const proceed=()=>{
      ...
      const _savedAw   = {...AW};
      let added=0, replaced=0, aeAdded=0, aeReplaced=0;
      ...
      // Merge autoevals from snapshot
      (d.autoevals||[]).forEach(incoming=>{
        if(!incoming.distribuidor||!incoming.answers) return;
        const existIdx=autoevals.findIndex(
          a=>a.distribuidor===incoming.distribuidor && a.edicion===incoming.edicion
        );
        if(existIdx>=0){ autoevals[existIdx]=incoming; aeReplaced++; }
        else { autoevals.push(incoming); aeAdded++; }
      });
      ...
      showToast(
        '✅ Fusión — Edición '+anio+
        ': '+added+' EV agregadas, '+replaced+' EV actualizadas'
        +(aeAdded||aeReplaced?' | '+aeAdded+' AE nuevas, '+aeReplaced+' AE actualizadas':'')
      );
...
(function(){
  ...
  if(snapText && snapText !== '/* __SNAP_PLACEHOLDER__ */'){
    try{
      ...
      if(d.autoevals&&d.autoevals.length) autoevals.push(...d.autoevals);
